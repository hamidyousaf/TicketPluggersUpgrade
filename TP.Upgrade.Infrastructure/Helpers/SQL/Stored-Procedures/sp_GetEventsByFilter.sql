CREATE OR ALTER PROCEDURE [dbo].[GetEventsByFilter]
-- Parameters.
	@radiusFrom int,
	@radiusTo int,
	@latitude varchar(20),
...
WITH CTE2 AS (
  ...
  FROM 
      (SELECT  
			EE.Id,Name,Description,EventStartUTC,LEFT(EventStartTime,5) AS eventStartTime,ImageURL,IsHotEvent,VenueID, 
			EE.TimeZone,TicketMaxPrice,TicketMinPrice, TicketAvailable AS AvailableTickets,TicketUploaded,EventSearchCount,SegmentID, 
			Event_ID,Notes,EventStatusId,EventEndUTC,EV.Latitude,EV.Longitude,EV.VenueName,EV.CountryCode,EV.City 
		FROM Events EE INNER JOIN Venues EV ON EV.Id = EE.VenueId WHERE Published = 1 AND EE.IsDeleted = 0  AND EventStartUTC > GETDATE()
       AND (EV.CountryCode=@countryCode OR @countryCode=null ) AND (( @eventStartDate IS NOT NULL AND CONVERT(DATE, EventStartUTC) >= CONVERT(DATE, @eventStartDate) ) OR (@eventStartDate IS NULL))
  ...
  GROUP BY Name, Id, Name, Description, EventStartUTC, eventStartTime, ImageURL, IsHotEvent, VenueID, VenueName, City, CountryCode,
	TimeZone, TicketMaxPrice, TicketMinPrice, AvailableTickets, TicketUploaded, EventSearchCount, SegmentID, Event_ID, Notes, EventStatusId,
	EventEndUTC, distance
	ORDER BY
	  CASE 
